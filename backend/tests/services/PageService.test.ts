...
describe('PageService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new PageService(mockPortfolioRepo, mockPortfolioVersionRepo, mockPageRepo);
  });

  describe('createPage', () => {
    it('should throw an error if the portfolio is not found', async () => {
      mockPortfolioRepo.findOne.mockResolvedValueOnce(undefined);
      await expect(service.createPage('Page 1', 'url', 1)).rejects.toThrow('Portfolio not found');
    });
    ...
    it('should throw an error if the page already exists', async () => {
      ...
      await expect(service.createPage('Page 1', 'url', 1)).rejects.toThrow('Page already exists');
    });
    ...
    it('should throw an error if the draft version already has 3 pages', async () => {
      ...
      await expect(service.createPage('Page 1', 'url', 1)).rejects.toThrow('Draft version already has 3 pages');
    });
  ...
  describe('getAllPagesForAPortfolioVersion', () => {
    it('should throw an error if the portfolio is not found', async () => {
      ...
      await expect(service.getAllPagesForAPortfolioVersion(1)).rejects.toThrow('Portfolio not found');
    });
  });
});
