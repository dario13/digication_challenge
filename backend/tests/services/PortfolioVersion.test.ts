...
import { PortfolioVersionService } from '../../src/services/PortfolioVersionService';
import { mockPageRepo, mockPortfolioRepo, mockPortfolioVersionRepo } from './mockedRepos';
...
describe('PortfolioVersionService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new PortfolioVersionService(mockPortfolioRepo, mockPortfolioVersionRepo, mockPageRepo);
  });

  describe('createSnapshotFromDraft', () => {
    it('should throw an error if too many pages are provided', async () => {
      await expect(service.createSnapshotFromDraft(1, [1, 2, 3, 4, 5, 6])).rejects.toThrow(
        'Too many pages, limit is 5'
      );
    });

    it('should throw an error if the portfolio is not found', async () => {
      mockPortfolioRepo.findOne.mockResolvedValueOnce(undefined);
      await expect(service.createSnapshotFromDraft(1, [1, 2, 3])).rejects.toThrow('Portfolio not found');
    });
    ...
    it('should create a snapshot from a draft', async () => {
      ...
      expect(mockPortfolioVersionRepo.create).toHaveBeenCalledWith({
        portfolio,
  ...
  describe('createSnapshotPortfolioVersion', () => {
    it('should throw an error if the portfolio is not found', async () => {
      mockPortfolioRepo.findOne.mockResolvedValueOnce(undefined);
      await expect(service.createSnapshotPortfolioVersion(1)).rejects.toThrow('Portfolio not found');
    });
